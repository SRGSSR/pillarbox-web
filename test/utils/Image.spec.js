...
const url = 'https://image.to.scale.ch/img.jpg';

describe('Image', () => {
  describe('scale', () => {
    it('should return undefined if the no parameters are provided', () => {
      expect(Image.scale()).toBeUndefined();
    });
    it('should return an url starting with the schema of the IL image service', () => {
      const imageUrl = Image.scale({ url });

      expect(imageUrl.startsWith('https://il.srgssr.ch/images/?imageUrl=')).toBe(
        true
      );
    });

    it('should apply the default image width and format', () => {
      const imageUrl = new URL(Image.scale({ url }));
